...
```{r}
# Use 'ddply' to calculate, for each month, means and standard deviations for daily air temperature and precipitation
monthlyData <- ddply(climate,
                     "Month",
                     function(x){ 
...
```{r}
# Use 'transform' within 'ddply' to split your data into subsets, perform a calculation on each subset, and add the results to a copy of your input data frame as a new column
x <- ddply(climate, 
           "Month", 
           transform, 
...
# 'Mutate' works similarly to 'transform', but allows you to do calculations within 'ddply' using columns you just created
x <- ddply(climate, 
           "Month", 
           mutate, 
...
d_ply(climate, 
      ...
      boxplot(AvgAirTemp, 
              ...
              ylab="Mean Air Temperature (degrees C)"))
```

### Using other types of functions within 'ddply'
```{r}
# Use a linear model to examine how precipitation changes with air temperature within each month
...
### Create graphs using 'ddply' output
